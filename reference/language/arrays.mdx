...
- **Nested literals stay manual.** `new T[a][b]` exists for heap allocation, but the equivalent fully-initialized nested literal (e.g. `[[1, 2], [3, 4]]` as a single 2D-shaped initializer) still requires writing each row literal explicitly.
