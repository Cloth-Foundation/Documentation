...
This page documents Cloth’s **signed, fixed-width, two’s-complement** integer types. Use these when you need exact whole-number arithmetic with well-defined overflow and bitwise behavior.
...
Cloth supports decimal, binary, octal, and hexadecimal integer literals. Underscores are allowed for readability and don’t affect the value.
...
Cloth provides **checked** behavior in Debug and **wrapping** behavior in Release by default, plus explicit helpers for all modes:
...
Use Cloth’s built-ins for output; formatting supports bases.
